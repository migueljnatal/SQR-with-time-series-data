## MC Simulation -  Location-shift model 2 Galvao (2013) - 50 quantile levels
...
for(j in 1:nrep){ 
  ...
  for (i in 1:length(taus)){
    ...
    # Demais niveis de $\eta$ = h 'rule of thumb' 
    h_3[i,j]        = h[i,j]/4
    h_2[i,j]        = h[i,j]/2
    h2[i,j]         = h[i,j]*2 
    h3[i,j]         = h[i,j]*4
...
for(s in 1:length(taus)){
  ...
  intercept_rq_mse[s] = mean((A[s,1,] - qt(taus[s],df=3))^2)
  intercept_cq_mse[s] = mean((C[s,1,] - qt(taus[s],df=3))^2)
