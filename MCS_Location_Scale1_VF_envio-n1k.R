## MC Simulation -  Location-shift-scale model 1 Galvao (2013) - 50 quantile levels

setwd("C:/Users/Miguel/OneDrive - UFRGS/Mestrado PPGEst/DISSERTA√á√ÉO")
...
path =  paste("C://Users//Miguel//OneDrive - UFRGS//Mestrado PPGEst//DISSERTA√á√ÉO//")
...
for(j in 1:nrep){ 
  ...
  for (i in 1:length(taus)){
    ...
    # Other levels of $\zeta$ = h 'rule of thumb'
    h_3[i,j]        = h[i,j]/4
    h_2[i,j]        = h[i,j]/2
    h2[i,j]         = h[i,j]*2
    h3[i,j]         = h[i,j]*4
...
for(s in 1:length(taus)){
  ...
  intercept_rq_bias[s] = mean(A[s,1,]) - 0
  intercept_cq_bias[s] = mean(C[s,1,]) - 0
  alphas1_rq_bias[s] = mean(A[s,2,]) - 0.5
  alphas1_cq_bias[s] = mean(C[s,2,]) - 0.5
  betas1_rq_bias[s] = mean(A[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas1_cq_bias[s] = mean(C[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_rq_bias[s] = mean(A[s,4,]) - 0.5
  betas2_cq_bias[s] = mean(C[s,4,]) - 0.5 
  # h*2
  intercept_cq2_bias[s] = mean(C2[s,1,]) - 0
  alphas1_cq2_bias[s] = mean(C2[s,2,]) - 0.5
  betas1_cq2_bias[s] = mean(C2[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq2_bias[s] = mean(C2[s,4,]) - 0.5 
  # h*4
  intercept_cq3_bias[s] = mean(C3[s,1,]) - 0
  alphas1_cq3_bias[s] = mean(C3[s,2,]) - 0.5
  betas1_cq3_bias[s] = mean(C3[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq3_bias[s] = mean(C3[s,4,]) - 0.5
  # h/4
  intercept_cq_3_bias[s] = mean(C_3[s,1,]) - 0
  alphas1_cq_3_bias[s] = mean(C_3[s,2,]) - 0.5
  betas1_cq_3_bias[s] = mean(C_3[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq_3_bias[s] = mean(C_3[s,4,]) - 0.5
  # h/2
  intercept_cq_2_bias[s] = mean(C_2[s,1,]) - 0
  alphas1_cq_2_bias[s] = mean(C_2[s,2,]) - 0.5
  betas1_cq_2_bias[s] = mean(C_2[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq_2_bias[s] = mean(C_2[s,4,]) - 0.5 
  # h*8
  intercept_cq8_bias[s] = mean(C8[s,1,]) - 0
  alphas1_cq8_bias[s] = mean(C8[s,2,]) - 0.5
  betas1_cq8_bias[s] = mean(C8[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq8_bias[s] = mean(C8[s,4,]) - 0.5 
  # h*16
  intercept_cq16_bias[s] = mean(C16[s,1,]) - 0
  alphas1_cq16_bias[s] = mean(C16[s,2,]) - 0.5
  betas1_cq16_bias[s] = mean(C16[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq16_bias[s] = mean(C16[s,4,]) - 0.5
  # h/16
  intercept_cq_16_bias[s] = mean(C_16[s,1,]) - 0
  alphas1_cq_16_bias[s] = mean(C_16[s,2,]) - 0.5
  betas1_cq_16_bias[s] = mean(C_16[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq_16_bias[s] = mean(C_16[s,4,]) - 0.5
  # h/8
  intercept_cq_8_bias[s] = mean(C_8[s,1,]) - 0
  alphas1_cq_8_bias[s] = mean(C_8[s,2,]) - 0.5
  betas1_cq_8_bias[s] = mean(C_8[s,3,]) - (0.5+0.2*qnorm(taus[s],0,1))
  betas2_cq_8_bias[s] = mean(C_8[s,4,]) - 0.5

  # MSE 1st - 50th quantiles
  intercept_rq_mse[s] = mean((A[s,1,] - 0)^2)
  intercept_cq_mse[s] = mean((C[s,1,] - 0)^2)
  alphas1_rq_mse[s] = mean((A[s,2,] - 0.5)^2)
  alphas1_cq_mse[s] = mean((C[s,2,] - 0.5)^2)
  betas1_rq_mse[s] = mean((A[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas1_cq_mse[s] = mean((C[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_rq_mse[s] = mean((A[s,4,] - 0.5)^2)
  betas2_cq_mse[s] = mean((C[s,4,] - 0.5)^2)
  # h*2
  intercept_cq2_mse[s] = mean((C2[s,1,] - 0)^2)
  alphas1_cq2_mse[s] = mean((C2[s,2,] - 0.5)^2)
  betas1_cq2_mse[s] = mean((C2[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq2_mse[s] = mean((C2[s,4,] - 0.5)^2)
  # h*3
  intercept_cq3_mse[s] = mean((C3[s,1,] - 0)^2)
  alphas1_cq3_mse[s] = mean((C3[s,2,] - 0.5)^2)
  betas1_cq3_mse[s] = mean((C3[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq3_mse[s] = mean((C3[s,4,] - 0.5)^2)
  # h*1/3
  intercept_cq_3_mse[s] = mean((C_3[s,1,] - 0)^2)
  alphas1_cq_3_mse[s] = mean((C_3[s,2,] - 0.5)^2)
  betas1_cq_3_mse[s] = mean((C_3[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  ...
  betas1_cq_2_mse[s] = mean((C_2[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq_2_mse[s] = mean((C_2[s,4,] - 0.5)^2)
  # h*8
  intercept_cq8_mse[s] = mean((C8[s,1,] - 0)^2)
  alphas1_cq8_mse[s] = mean((C8[s,2,] - 0.5)^2)
  betas1_cq8_mse[s] = mean((C8[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq8_mse[s] = mean((C8[s,4,] - 0.5)^2)
  # h*16
  intercept_cq16_mse[s] = mean((C16[s,1,] - 0)^2)
  alphas1_cq16_mse[s] = mean((C16[s,2,] - 0.5)^2)
  betas1_cq16_mse[s] = mean((C16[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq16_mse[s] = mean((C16[s,4,] - 0.5)^2)
  # h/16
  intercept_cq_16_mse[s] = mean((C_16[s,1,] - 0)^2)
  alphas1_cq_16_mse[s] = mean((C_16[s,2,] - 0.5)^2)
  betas1_cq_16_mse[s] = mean((C_16[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
  betas2_cq_16_mse[s] = mean((C_16[s,4,] - 0.5)^2)
  # h/8
  intercept_cq_8_mse[s] = mean((C_8[s,1,] - 0)^2)
  alphas1_cq_8_mse[s] = mean((C_8[s,2,] - 0.5)^2)
  betas1_cq_8_mse[s] = mean((C_8[s,3,] - (0.5+0.2*qnorm(taus[s],0,1)))^2)
...
for(s in 1:length(taus)){
  ...
  real_alpha0[s] = 0
  real_alpha1[s] = 0.5
  real_theta1[s] = (0.5+0.2*qnorm(taus[s],0,1))
  real_theta2[s] = 0.5
...
# 
# coefs_alphas0_vpq = data.frame(alphas0_rq_vpq, alphas0_cq_vpq,
...
#                                alphas1_cq2_vpq,alphas1_cq3_vpq,alphas1_cq_3_vpq,alphas1_cq_2_vpq,
#                                alphas1_cq8_vpq,alphas1_cq16_vpq,alphas1_cq_16_vpq,alphas1_cq_8_vpq)
# coefs_thetas1_vpq = data.frame(thetas1_rq_vpq, thetas1_cq_vpq,
...
#                               betas2_cq8_vpq,betas2_cq16_vpq,betas2_cq_16_vpq,betas2_cq_8_vpq)
# 
# path_outputs = "C:/Users/MIGUEL/OneDrive - UFRGS/Mestrado PPGEst/DISSERTA«√O/Novas SimulaÁıes/outputs_checkpoints/"
# path_lambda = choice
# path_sample = as.character(Tsize-100)
